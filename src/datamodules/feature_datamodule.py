...
from src.datamodules.ubc_datamodule import UbcDataModule, UbcDataset, make_ubc_dataset
...
class ColonAllDataModule(ColonAllDataModule):
    ...
    def setup(self, stage=None):
        # test for the trained dataset
        if stage != "fit" and stage is not None:
            train_df1, _ = bring_colon_dataset_csv(
                datatype="COLON_MANUAL_512", stage=None
            )
...
class UbcDataModule(UbcDataModule):
    ...
    def setup(self, stage=None):
        # test for the trained dataset
        if stage != "fit" and stage is not None:
            train_set, _ = make_ubc_dataset(stage="train")
            if self.hparams.data_ratio < 1.0:
                train_set = (
                    pd.DataFrame(train_set, columns=["path", "class"])
                    .groupby("class")
...
class ProstateDataModule(ProstateDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage != "fit" and stage is not None:
            ...
            train_set2, _ = make_ubc_dataset(stage="train")
